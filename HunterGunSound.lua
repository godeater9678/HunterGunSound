...
local function CheckMachineGunMode()
    ...
    if #attackTimes >= threshold then
        if not machineGunActive then
            ...
            -- 머신건 발동조건이 되었을 때 사운드 재생 (bow/crossbow는 many-arrows-flying.ogg)
            if _HunterGunSounds.machineGunEnabled and currentTime - lastMachineGunTime > 0.1 then
                local soundFile
                if Bow or CBow then
                    soundFile = GetSoundFileName("Interface\\AddOns\\HunterGunSound\\sounds\\many-arrows-flying")
                else
                    soundFile = GetSoundFileName("Interface\\AddOns\\HunterGunSound\\sounds\\machinegun")
                end
...
local function PlayAttackSound()
    ...
    if isMachineGun and _HunterGunSounds.machineGunEnabled then
        -- 머신건 모드: bow/crossbow 착용 시 many-arrows-flying.ogg, 그 외에는 machinegun.ogg 재생
        ...
        local soundFile = GetSoundFileName(filen .. "1")
        PlaySoundFile(soundFile, 0, 0.5)
        
        -- Play echo effect if enabled (but not for bow/crossbow)
        if _HunterGunSounds.echoEnabled and not (Bow or CBow) then
...
local function onEvent(self, event, ...)
	...
	elseif _HunterGunSounds.enabled == "on" and (event == "UNIT_SPELLCAST_START") and (arg1 == "player") then
		if (arg3==56641 or arg3==19434) then
			ReloadS=999999998
			if Bow then
				PlaySoundFile(GetSoundFileName(filer .. "1"))
			end
		else
			ReloadS=999999999
		end
...
versionText:SetText("|cFFFFFFFFVersion 1.4.0|r")
